...
class CarouselWishlist extends Component {
   constructor(props) {
      super(props)
   ...
   handleButton(movie) {
      ...
      if(this.props.movies.length > 3) {
         fire.database().ref(`users/${fire.auth().currentUser.uid}/movies/wishlist`)
            .orderByChild("movieID")
            .equalTo(movie.details.id)
            .once('value', snapshot => {
                var key = Object.keys(snapshot.val())[0]
                console.log(key);
                this.props.removeMovieFromLibrary(fire.auth().currentUser.uid, key, 'wishlist');
